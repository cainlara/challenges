 ...
 * a 70 feet lead but B speed is 850 feet per hour. 
 * How long will it take B to catch A ?
 * 
 * More generally: given two speeds v1 (A speed, integer > 0) 
 * and v2 (B speed, integer > 0) and a lead g (integer > 0) 
 * how long will it take B to catch A?
 * 
 * The result will be an array [h, mn, s] where 
 * h, mn, s is the time needed in hours, minutes and seconds 
...
public class Tortoise {
	public static int[] race(int v1, int v2, int g) {
		...
		if (v1 < v2) {
			float feetPerSecond1 = v1 / 3600f;
			float feetPerSecond2 = v2 / 3600f;
			float time = g / (feetPerSecond2 - feetPerSecond1);

			int hours = (int) time / 3600;
			int minutes = (int) (time - hours * 3600) / 60;
			int seconds = (int) time - (hours * 3600) - (minutes * 60);

			result = new int[] { hours, minutes, seconds };
		}
